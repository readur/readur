...
async fn create_test_app_state() -> Result<Arc<AppState>> {
    ...
    config.upload_path = "./test_uploads".to_string();
    config.watch_folder = "./test_watch".to_string();
    config.concurrent_ocr_jobs = 1;
    config.ocr_timeout_seconds = 30;
    ...
    let queue_service = Arc::new(readur::ocr::queue::OcrQueueService::new(db.clone(), db.pool.clone(), 1, file_service.clone(), 100, 100, 300));
